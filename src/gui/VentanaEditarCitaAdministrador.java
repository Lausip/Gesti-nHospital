...
public class VentanaEditarCitaAdministrador extends JFrame {
	...
	private JComboBox<String> cbCausas;
	 ...
	 * Create the frame.
	 */
	public VentanaEditarCitaAdministrador(Cita cit) {
	...
	private JLabel getLblDatosDeLacita() {
		if (lblDatosDeLacita == null) {
			lblDatosDeLacita = new JLabel("Modificar Campos");
	...
	private JPanel getPanelDatosPaciente() {
		if (panelDatosPaciente == null) {
			...
			panelDatosPaciente.add(getPnTotal());
	...
	private JPanel getPnPaciente() {
		if (pnPaciente == null) {
			...
			pnPaciente.setLayout(new BorderLayout(0, 0));
			pnPaciente.add(getPnCausa(), BorderLayout.CENTER);
		}
		return pnPaciente;
	}
	...
	private JPanel getPnTotal() {
		if (pnTotal == null) {
			pnTotal = new JPanel();
			pnTotal.setBorder(new LineBorder(Color.LIGHT_GRAY, 2));
			pnTotal.setBounds(20, 221, 325, 74);
			pnTotal.setLayout(new BorderLayout(0, 0));
			pnTotal.add(getLblCausas(), BorderLayout.NORTH);
			pnTotal.add(getPnPaciente(), BorderLayout.CENTER);
		}
		return pnTotal;
	}
	...
	private JPanel getPnCausa() {
		if (pnCausa == null) {
			pnCausa = new JPanel();
			pnCausa.setLayout(new BorderLayout(0, 0));
			pnCausa.add(getCbCausas(), BorderLayout.CENTER);
		}
		return pnCausa;
	}
	...
	private JLabel getLblCausas() {
		if (lblCausas == null) {
			...
			lblCausas.setBackground(Color.WHITE);
			lblCausas.setFont(new Font("Tahoma", Font.PLAIN, 19));
		}
		return lblCausas;
	}
	...
	private JRadioButton getRdbtnHaAcudido() {
		if (rdbtnHaAcudido == null) {
			rdbtnHaAcudido = new JRadioButton("Es urgente");
	...
	protected void actualizarActivos() {
		...
		if(rdbtnHaAcudido.isEnabled()) {
			cit.urgente=rdbtnHaAcudido.isSelected();	
			dbc.actualizarUrgenteCita(cit);
		}
